# Кроссплатформенное приложение для мониторинга криптовалютного рынка

`Crypto Informer` — курсовой проект OTUS: кроссплатформенное Flutter-приложение для мониторинга криптовалютного рынка с курсами, графиками, избранным, уведомлениями и offline-first поведением.
...
- Проект содержит конфигурации для Android, iOS, macOS, Linux и Web.
...
Сборка под конкретную платформу — стандартными командами Flutter (`flutter run -d android`, `flutter run -d ios`, `flutter run -d macos`, `flutter run -d linux`, `flutter run -d chrome` и т.д.).
