...
- **`storage/sql/`** — оболочка БД (открытие, версия, миграции) отдельно от логики по таблицам. Каждая таблица или связный кэш оформляется парой **интерфейс + реализация** в подкаталоге вроде `tables/` (один файл — одна ответственность), чтобы не раздувать файл базы.
