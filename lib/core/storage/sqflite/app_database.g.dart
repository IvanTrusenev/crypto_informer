...
class _$AppDatabase extends AppDatabase {
  ...
  MarketAssetsCacheDao? _marketAssetsCacheDaoInstance;
    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
          'CREATE TABLE IF NOT EXISTS `market_assets_cache` (`cache_key` TEXT NOT NULL, `id` TEXT NOT NULL, `vs_currency` TEXT NOT NULL, `sort_order` INTEGER NOT NULL, `updated_at` INTEGER NOT NULL, `symbol` TEXT NOT NULL, `name` TEXT NOT NULL, `current_price_usd` REAL NOT NULL, `price_change_percent_24h` REAL NOT NULL, `market_cap_usd` REAL, `image_url` TEXT, PRIMARY KEY (`cache_key`))',
        );
        await database.execute(
  ...
  MarketAssetsCacheDao get marketAssetsCacheDao {
    return _marketAssetsCacheDaoInstance ??= _$MarketAssetsCacheDao(
      database,
      changeListener,
    );
  }
...
class _$MarketAssetsCacheDao extends MarketAssetsCacheDao {
  _$MarketAssetsCacheDao(this.database, this.changeListener)
    : _queryAdapter = QueryAdapter(database),
      _marketAssetCacheRecordInsertionAdapter = InsertionAdapter(
        database,
        'market_assets_cache',
        (MarketAssetCacheRecord item) => <String, Object?>{
  ...
  final InsertionAdapter<MarketAssetCacheRecord>
  _marketAssetCacheRecordInsertionAdapter;

  @override
  Future<List<MarketAssetCacheRecord>> findByVsCurrency(
    String vsCurrency,
  ) async {
    return _queryAdapter.queryList(
      'SELECT *     FROM market_assets_cache     WHERE vs_currency = ?1     ORDER BY sort_order ASC',
      mapper: (Map<String, Object?> row) => MarketAssetCacheRecord(
  ...
  Future<void> deleteByVsCurrency(String vsCurrency) async {
    await _queryAdapter.queryNoReturn(
      'DELETE FROM market_assets_cache WHERE vs_currency = ?1',
      arguments: [vsCurrency],
    );
  }

  @override
  Future<void> insertRows(List<MarketAssetCacheRecord> rows) async {
    await _marketAssetCacheRecordInsertionAdapter.insertList(
      rows,
  ...
  Future<void> replaceRows(
    ...
    if (database is sqflite.Transaction) {
      await super.replaceRows(vsCurrency, rows);
    } else {
      await (database as sqflite.Database).transaction<void>((
        ...
        await transactionDatabase.marketAssetsCacheDao.replaceRows(
          vsCurrency,
          rows,
        );
      });
...
class _$CoinDetailCacheDao extends CoinDetailCacheDao {
  ...
  @override
  Future<void> upsert(CoinDetailCacheRecord row) async {
